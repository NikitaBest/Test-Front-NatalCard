    ...
    <div className="w-full max-w-xl mx-auto px-4 pb-8">
      {blocks.map((block, i) => (
        <div key={i} className="mb-8">
          <h2 className="font-mono text-xl font-normal text-gray-800 mb-4 mt-6 text-left">{block.title}</h2>
          {block.tips && block.tips.length > 0 && (
            <ul className="mb-4">
              {block.tips.map((tip, j) => (
                <li key={j} className="flex items-center gap-2 text-sm mb-2 text-gray-900">
          ...
          <div
            className="text-sm text-gray-900 font-sans whitespace-pre-line"
